...
const mistralSystemPrompt = "You are KAVIYA-MD, an intelligent bot. Someone mentioned your owner. Respond with helpful and friendly assistance.";
...
const mistral = async (m, Matrix) => {
    ...
    if (isBotOrOwnerMentioned) {
        ...
        if (text === `@${botNumber}` || text === `@${ownerNumber}`) {
            await Matrix.sendMessage(m.from, { audio: { url: 'https://i.ibb.co/N7jMyLg/739298f4cc428747.jpg' } }, { quoted: m });
            return;
        }
